...
1. 客户端检查hosts文件是否有网址映射关系
2. 若无，查找本地DNS解析器缓存
3. 若无，向DNS服务器查询域名的IP地址
...
解析器和DNS服务器将最新了解到的信息暂时保存在缓存中，缓存时间由TTL参数指定，减少每次查询的性能消耗
...
NAT和NART都需要具备超时释放机制，当计时器归零后，转换表资源会被释放，该资源就可以被新的请求所使用，达到循环使用的目的。[Timeout](https://cloud.tencent.com/developer/news/232308)
