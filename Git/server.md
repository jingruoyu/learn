...
目前很多Git服务器都使用SSH公钥进行认证。为了向Git服务器提供SSH公钥，用户必须事先生成一份，此过程在所有OS上都是相似的

默认情况下用户的SSH秘钥存储在`~/.ssh`目录下，从该目录中可以确认自己是否已经拥有秘钥

* `name.pub`为公钥，另一个对应的同名文件为私钥
* 如果没有这样的文件，则可以使用`ssh-keygen`程序创建他们
...
* 在服务器上创建一个操作系统用户git
* 将授信用户的public key添加到git用户的`authorized_keys`文件中
* 创建一个bare repo，可以使用`git init --bare`进行

之后授信用户即可将此仓库作为本地repo的远程repo进行push

`git remote add origin git@gitserver:/srv/git/project.git`

此处需要将`gitserver`域名指向该部署服务器，本地或内网重定向都可以

这种方法每次添加一个新项目都需要登录服务器，添加一个bare repo

此时所有开发者都可以clone此仓库，并push各自的commit

### 权限限制

此时所有授权用户都能以系统用户身份登录服务器并获得一个普通shell。如果想对此加以限制，可以使用`git-shell`替代bash或者csh，使其变成该用户的登录shell

限制后，用户`git`就只能利用`SSH`连接对`Git`仓库进行推送和拉取操作，而不能登录机器并取得普通`shell`

## Git守护进程

对于快速且无需授权的Git数据访问，建立基于Git守护进程的仓库是一个理想选择。但是其不包含授权服务，任何通过该协议管理的内容都将在网络上公开

* 运行在防火墙外的服务器，应该只对公开的**只读**项目服务
* 运行在防火墙内的服务器，可用于支撑大量参与人员或自动系统的**只读**项目服务

这样可以省去逐一配置SSH公钥

核心操作

* 将Git命令以守护进程的方式执行并实现监控
* 在允许无授权访问的仓库下添加配置文件，允许Git提供无需授权的项目访问服务

## Smart HTTP

Smart Http协议可以同时实现授权访问与无授权访问

设置 Smart HTTP 一般只需要在服务器上启用一个 Git 自带的名为 git-http-backend 的 CGI 脚本

## GitWeb

GitWeb是一个Git项目网页查看器搭建工具，可以在线查看版本库

* 轻量级：可以在项目中使用Lighttpd或webrick，进行项目展示
* 完整版：使用GitWeb指定版本库位置，然后运行对应脚本，配置域名，即可访问

## GitLab

GitLab相比于GitWeb更复杂，功能更全面

* member
* group
* access
* 合并请求
	* committer可以正常创建分支，提交commit
	* 非committer可以通过fork repo，之后提交一个从fork后的repo到主仓库的请求

优点：图形化界面

## 总结

* 多种协议：smartHTTP，GIT，SSH
* 不同协议优缺点

	* smartHTTP：有权限控制，一个URL，同时提供有权限与无权限
	* GIT：无权限控制，架设简单，一般只读，速度快
	* SSH：数据加密，安全高效，不支持匿名访问
