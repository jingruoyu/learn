...
上例中，匿名函数this指向window，但是其作用域链依然在show函数下面，故this.x指向window.x，x指向show中的x
...
箭头函数不适用于
* 需要使用动态this的场景
* 定义对象方法时，不能直接使用箭头函数。因为对象本身无法形成一个作用域,导致箭头函数中的this会直接使用更外层的this,而不是指向对象自身
